...
class FileGenerator:
    ...
    def generate_files(self) -> None:
        ...
        # for mode, region_map in world.region_packs.items():
        #     regions_seen_keys = region_map.region_list
        ...
        #     code_region_connections = [ast.unparse(self.ast_generator.create_ast_call_region_connection(item)) for item in region_map.region_connections]
        #     code_region_connections = ",\n".join(code_region_connections)

        #     code_excluded_regions = ast.unparse(
        #         ast.List([ast.Constant(x) for x in self.ctx.rando_data["regions"][mode]["excludedRegions"]]))

        #     code_region_pack_list.append({
        #         "name": mode,
        #         "region_list": code_region_list,
        #         "region_connections": code_region_connections,
        #         "starting_region": self.ctx.rando_data["regions"][mode]["startingRegion"],
        #         "goal_region": self.ctx.rando_data["goalRegion"][mode],
        #         "excluded_regions": code_excluded_regions
        #     })

        code_region_lists = { mode: emit_list([ast.Constant(r) for r in pack.region_list]) for mode, pack in world.region_packs.items() }
        code_region_connections = { mode: emit_list([self.ast_generator.create_ast_call_region_connection(c) for c in pack.region_connections]) for mode, pack in world.region_packs.items() }

        regions_complete = template.render(
            region_packs=world.region_packs,
            region_lists=code_region_lists,
            region_connections=code_region_connections,
            modes_string=", ".join(
